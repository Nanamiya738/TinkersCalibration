...
public class IgneousModifier extends Modifier implements ModifierRemovalHook, TooltipModifierHook, ToolDamageModifierHook, MeleeDamageModifierHook, BreakSpeedModifierHook, InventoryTickModifierHook, ConditionalStatModifierHook{
    ...
    public void onInventoryTick(@Nonnull IToolStackView tool, ModifierEntry modifier, @Nonnull Level world, @Nonnull LivingEntity holder, int itemSlot, boolean isSelected, boolean isCorrectSlot, ItemStack stack) {
        ...
        if (!world.isClientSide && holder.tickCount % 100 == 0 && isSelected && holder.isOnFire()) {
            if (persistentData.getFloat(KEY) <= 5 * modifier.getLevel() && RANDOM.nextFloat() <= 0.6f * modifier.getLevel()) {
    ...
    public void addTooltip(IToolStackView tool, ModifierEntry modifier, @Nullable Player player, List<Component> tooltip, slimeknights.mantle.client.TooltipKey tooltipKey, TooltipFlag tooltipFlag) {
        if (player != null) {
            ...
            if (persistentData.contains(KEY, 5)) {
                float value = persistentData.getFloat(KEY);
                TooltipModifierHook.addPercentBoost(modifier.getModifier(), Component.translatable("modifier.tinkerscalibration.modifier.igneous.attack_damage"), value * 0.1, tooltip);
                if (harvest) {
                    TooltipModifierHook.addPercentBoost(modifier.getModifier(), Component.translatable("modifier.tinkerscalibration.modifier.igneous.mining_speed"), value * 0.1, tooltip);
                } else {
                    TooltipModifierHook.addPercentBoost(modifier.getModifier(), Component.translatable("modifier.tinkerscalibration.modifier.igneous.draw_speed"), value * 0.1, tooltip);
                }
                if (value >= modifier.getLevel()) {
                    ...
                    TooltipModifierHook.addPercentBoost(modifier.getModifier(), UNBREAKING, value / 10, tooltip);
                }
