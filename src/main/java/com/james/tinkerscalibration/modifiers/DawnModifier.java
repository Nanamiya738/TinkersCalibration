...
public class DawnModifier extends Modifier implements TooltipModifierHook, MeleeDamageModifierHook, BreakSpeedModifierHook, ConditionalStatModifierHook, ProjectileHitModifierHook {
    ...
    public void onBreakSpeed(@Nonnull IToolStackView tool, ModifierEntry modifier, @Nonnull PlayerEvent.BreakSpeed event, @Nonnull Direction sideHit, boolean isEffective, float miningSpeedModifier) {
        ...
        if (world.getDayTime() < 24000L && world.getDayTime() > 22200L || player.getCommandSenderWorld().dimension().equals(SkiesDimensions.everdawnKey())) {
    ...
    public float getMeleeDamage(@Nonnull IToolStackView tool, ModifierEntry modifier, @Nonnull ToolAttackContext context, float baseDamage, float damage) {
        ...
        if(player != null){
            Level world = player.getCommandSenderWorld();
            if (world.dayTime() < 24000L && world.dayTime() > 22200L || player.getCommandSenderWorld().dimension().equals(SkiesDimensions.everdawnKey())) {
    ...
    public float modifyStat(IToolStackView tool, ModifierEntry modifier, LivingEntity living, FloatToolStat stat, float baseValue, float multiplier) {
        ...
        if (stat == ToolStats.DRAW_SPEED) {
            if (world.getDayTime() < 24000L && world.getDayTime() > 22200L || living.getCommandSenderWorld().dimension().equals(SkiesDimensions.everdawnKey())) {
    ...
    public boolean onProjectileHitEntity(ModifierNBT modifiers, ModDataNBT persistentData, ModifierEntry modifier, Projectile projectile, EntityHitResult hit, @Nullable LivingEntity attacker, @Nullable LivingEntity target){
        if (attacker != null && (attacker.getCommandSenderWorld().getDayTime() < 24000L && attacker.getCommandSenderWorld().getDayTime() > 22200L || attacker.getCommandSenderWorld().dimension().equals(SkiesDimensions.everdawnKey()))) {
