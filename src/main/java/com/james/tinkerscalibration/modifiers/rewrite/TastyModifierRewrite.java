...
public class TastyModifierRewrite extends Modifier implements GeneralInteractionModifierHook, OnAttackedModifierHook, ProcessLootModifierHook {
    ...
    private void eat(IToolStackView tool, ModifierEntry modifier, LivingEntity entity) {
        ...
        if (level > 0 && entity instanceof Player player && player.canEat(false)) {
            ...
            player.getFoodData().eat(level * 3, 2 / 3F);
            ModifierUtil.foodConsumer.onConsume(player, BACON_STACK.get(), level, 4F);
